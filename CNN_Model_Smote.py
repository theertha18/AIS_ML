...
def apply_smote(x_train, y_train, k_neighbors=1):
    """
    Applies SMOTE to the given training data.
    ...
    - y_train_smote: The training labels after applying SMOTE, one-hot encoded.
    """
    ...
    print(f"Minimum number of samples in any class: {min_samples}")
    y_train_classes = np.argmax(y_train, axis=1)
    class_distribution = Counter(y_train_classes)

    valid_classes = [class_label for class_label, count in class_distribution.items() if count > k_neighbors]
    valid_indices = [i for i, label in enumerate(y_train_classes) if label in valid_classes]
    ...
    # Now you can apply SMOTE on x_train_filtered and y_train_filtered
    smote = SMOTE(k_neighbors=k_neighbors)
    x_train_smote, y_train_smote = smote.fit_resample(x_train_filtered, y_train_filtered)
    num_actual_classes = y_train_smote.shape[1]
    y_test_class_indices = np.argmax(y_test[:, :num_actual_classes], axis=1)
    y_test_one_hot = to_categorical(y_test_class_indices, num_classes=num_actual_classes)

    return x_train_smote, y_train_smote, y_test_one_hot
...
def train_model(xtrain, xtest, ytrain, ytest):
    ...
    model.summary()
    # Fit the model
    model.fit(xtrain, ytrain, epochs=epochs, batch_size=batch_size, verbose=verbose, class_weight=class_weight_dict)
    # Evaluate model
...
if __name__ == "__main__":
    ...
    x_train_smote, y_train_smote, y_test_one_hot = apply_smote(x_train, y_train, k_neighbors=1)
    ...
    # Now use the (over)sampled data for training
    model = train_model(x_train_smote, x_test, y_train_smote, y_test_one_hot)
